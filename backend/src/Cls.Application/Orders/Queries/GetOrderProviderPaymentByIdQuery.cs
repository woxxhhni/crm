...
public record GetOrderProviderPaymentByIdQuery(int OrderId, int Id) : IRequest<OrderPaymentResponse>;
public class GetOrderProviderPaymentByIdQueryHandler(IUnitOfWork uow, IOrderContractEnricher enricher) : IRequestHandler<GetOrderProviderPaymentByIdQuery, OrderPaymentResponse>
{
    public async Task<OrderPaymentResponse> Handle(GetOrderProviderPaymentByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var payment = order.ProviderPayments.FirstOrDefault(x => x.Id == request.Id);
        if (payment == null)
