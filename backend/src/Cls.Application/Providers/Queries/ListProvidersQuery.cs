...
public record ListProvidersQuery(
    ...
    string? Phone = null,
    string? Address = null,
    PagedRequest? Paging = null) : IRequest<PagedResult<ProviderResponse>>;

public class ListProvidersQueryHandler(IUnitOfWork uow, IMapper mapper)
    : IRequestHandler<ListProvidersQuery, PagedResult<ProviderResponse>>
{
    public async Task<PagedResult<ProviderResponse>> Handle(ListProvidersQuery request, CancellationToken ct)
    {
        var q = uow.Providers.Query().AsNoTracking();
        ...
        return new PagedResult<ProviderResponse>
        {
            ...
            Total = paged.Total,
            Items = mapper.Map<IReadOnlyList<ProviderResponse>>(paged.Items)
        };
    }
}
