...
public class ExtraProviderOrderPaymentFileConfiguration : SoftDeletableEntityConfiguration<ExtraProviderOrderPaymentFile>
{
    public override void Configure(EntityTypeBuilder<ExtraProviderOrderPaymentFile> e)
    {
        base.Configure(e);
        e.ToTable("extra_provider_payment_files");
        e.Property(x => x.PaymentId).IsRequired().HasColumnName("extra_provider_payment_id");
        e.Property(x => x.FileId).IsRequired().HasColumnName("file_id");

        e.HasIndex(x => x.PaymentId).HasDatabaseName("idx_extra_provider_payment_files_payment_id");
        e.HasIndex(x => x.FileId).HasDatabaseName("idx_extra_provider_payment_files_file_id");
