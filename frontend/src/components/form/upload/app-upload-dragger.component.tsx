...
import { Upload, Button, message, Tooltip, Image, Modal, Spin, Typography } from 'antd';
...
function FilePreviewThumbnail({ file, onDelete, canDelete = true }: { file: any; onDelete: (file: any) => void; canDelete?: boolean }) {
  const { t } = useTranslate('common');
  ...
  // Generate thumbnail URL for images (from existing URL or local file)
  ...
  const handlePreview = (e: React.MouseEvent) => {
    ...
    } else if (canPreviewDoc && previewUrl) {
    ...
    <>
      ...
      >
            ...
            {(isImage || canPreviewDoc) && (
              <Tooltip title={t('preview')}>
                <Button type="primary" shape="circle" size="small" icon={<EyeOutlined />}
                  onClick={handlePreview} style={{ fontSize: 12 }} />
              </Tooltip>
            )}
            {canDelete && (
        ...
        <Tooltip title={file.name}>
          <div style={{
            padding: '5px 8px', borderTop: '1px solid #f3f4f6',
            fontSize: 10, color: '#374151', overflow: 'hidden',
...
/* ─── Props ─── */
interface AppUploadDraggerProps {
  fileList: any[];
...
/* ─── Main Component ─── */
export default function AppUploadDraggerComponent({
  fileList,
  onChange,
  onRemove,
  multiple = true,
  className = '',
  disabled = false,
  hintText = 'Recommended formats: JPG, PNG, PDF, Word, Excel, Pages, Numbers',
  accept = '.jpg,.jpeg,.png,.pdf,.doc,.docx,.xls,.xlsx,.csv,.pages,.numbers,.epub',
  isSmall = false,
  onDeleteExistingFile,
  showDeleteConfirm = false,
  canDelete = true,
}: AppUploadDraggerProps) {
  ...
  if (isSmall) {
    ...
    return (
      <div className={clsx('flex flex-col gap-3', className)}>
        <Upload {...smallUploadProps}>
          <div
            className='flex items-center justify-center gap-2 h-[40px] px-4 border border-dashed rounded-lg cursor-pointer hover:border-[#3B82F6] transition-colors'
            style={{ borderColor: '#D9D9D9', backgroundColor: '#FAFAFA', borderRadius: '8px' }}>
            <UploadOutlined style={{ color: '#666' }} />
            <span className='text-[12px] text-gray-600'>{t('clickOrDragFileToThisAreaToUpload')}</span>
          </div>
        </Upload>
        {fileList.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            {fileList.map((file: any) => (
              <FilePreviewThumbnail key={file.uid} file={file} onDelete={handleDeleteClick} canDelete={canDelete} />
            ))}
          </div>
        )}
  ...
  return (
    <>
      <Upload.Dragger
        ...
        onChange={onChange}
        showUploadList={false}
        className={clsx('w-full border rounded-md transition-all py-5 border-dashed border-gray-300', className)}
      >
        <p className='ant-upload-drag-icon'>
          <UploadOutlined style={{ fontSize: 24, color: '#3B82F6' }} />
        </p>
        <p className='ant-upload-text'>{t('clickOrDragFileToThisAreaToUpload')}</p>
        <p className='ant-upload-hint text-gray-400 text-xs'>{hintText}</p>
      </Upload.Dragger>
