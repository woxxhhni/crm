...
export default function ClientsAddFileModalComponent({ open, onClose, mode, initialValues }: Props) {
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: async () => {
      ...
      if (mode === 'editTitle' && groupId) {
        await service.updateFileGroupLabel(clientId, groupId, getValues('title') ?? '');
      } else if (mode === 'editFiles' && groupId) {
        // Only call API if there are new files to upload
        if (files.length > 0) {
          await service.addFilesToGroup(clientId, groupId, files);
        }
      } else {
        await service.createFileGroup(clientId, {
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['client-file-groups', clientId] });
      onClose();
    },
  });
  ...
  const handleDeleteExistingFile = useCallback(
    async (file: any) => {
      try {
        ...
        if (fileId) {
          await service.deleteFileFromGroup(clientId, fileId);
    ...
    [clientId, notification, queryClient, service, t]
  );
