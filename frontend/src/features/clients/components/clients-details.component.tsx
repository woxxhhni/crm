...
import ClientsAddFileModalComponent from '@/features/clients/components/clients-add-file-modal.component';
...
export default function ClientsDetailsComponent() {
  ...
  const service = useMemo(() => container.resolve(ClientsService), []);
  ...
  const { data: fileGroups = [] } = useQuery({
    queryKey: ['client-file-groups', id],
  ...
  const deleteFileGroupMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['client-file-groups', id] });
  ...
  if (isError) return <Typography.Text type='danger'>{t('errorLoadingClientData')}</Typography.Text>;
  ...
  return (
    <>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
              <Avatar
                src={client?.profileUrl || undefined}
                size={72}
                style={{
          ...
          <div style={{ padding: '24px 32px 28px' }}>
            <Row gutter={[48, 0]}>
              <Col xs={24} md={12}>
                <InfoRow icon={<PhoneOutlined />} label={t('phone')} value={client.phone} />
                <InfoRow icon={<MailOutlined />} label={t('email')} value={client.email} />
                <InfoRow icon={<GlobalOutlined />} label={t('website')} value={client.website} />
              </Col>
              <Col xs={24} md={12}>
                <InfoRow icon={<PhoneOutlined />} label={t('secondPhone')} value={client.secondPhone} />
                <InfoRow icon={<FileTextOutlined />} label={t('description')} value={client.description} />
                <InfoRow icon={<EnvironmentOutlined />} label={t('address')} value={client.address} />
              </Col>
            </Row>
          </div>
        </Card>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Tag color="blue" style={{ borderRadius: 6, margin: 0 }}>{t('title')}</Tag>
      ...
      {isEditOpen && <ClientsFormModalComponent open={isEditOpen} onClose={() => setIsEditOpen(false)} initialValues={client} />}

      {fileModal.isOpen && <ClientsAddFileModalComponent mode={fileModal.mode} open={fileModal.isOpen} onClose={closeFileModal} initialValues={fileModalInitialValues} />}
    </>
  );
}
