...
export default function ExtraProviderPaymentFormModal({ open, onClose, orderId, epId, paymentId, currency }: Props) {
    ...
    const { data: paymentDetail, isLoading: isDetailLoading } = useQuery({
        queryKey: ['extra-provider-payment-detail', orderId, paymentId],
        queryFn: () => service.getExtraProviderPaymentDetail(orderId, paymentId!),
    ...
    const createMutation = useMutation({
        mutationFn: async (data: PaymentForm) => {
            ...
            await service.createExtraProviderPayment(orderId, epId, dto);
        },
        onSuccess: async () => {
    ...
    const updateMutation = useMutation({
        mutationFn: async (data: PaymentForm) => {
            ...
            await service.updateExtraProviderPayment(orderId, paymentId!, dto);
        },
        onSuccess: async () => {
            notification.success({ message: t('paymentUpdatedSuccessfully') });
            await queryClient.invalidateQueries({
                queryKey: ['extra-provider-payments', String(orderId), String(epId)]
            });
            await queryClient.invalidateQueries({
                queryKey: ['extra-provider-payment-detail', String(orderId), String(paymentId)]
            });
            onClose();
        },
    });
